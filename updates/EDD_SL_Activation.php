...
function edd_beantweets_activate_license() 
{
	...
	$license = trim( get_option( 'edd_beantweets_activate_license' ) );
	...
	if( $status == '' OR $status == 'invalid' ) 
	{
		...
		$api_params = array( 
			...
			'license' 	=> $license, 
			'item_name' => urlencode( EDD_BEANTWEETS_NAME )
		);
		
		//CALL CUSTOM API
		$response = wp_remote_get( add_query_arg( $api_params, EDD_BEANTWEETS_TB_URL ), array( 'timeout' => 15, 'sslverify' => false ) );
